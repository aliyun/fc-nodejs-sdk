...
class Client {
   ...
   */
  getLayerVersionByArn(layerArn, options = {}, headers = {}) {
   ...
   * - official, Accept value: true or false.
   */
  putLayerACL(layerName, options = {}, headers = {}) {
   ...
   */
  async publishLayerVersionForBigCode(layerName, options = {}, headers = {}) {
    const {
      size,
      zipFilePath
    } = options.codeConfig || {};

    const { data: { CodeSizeLimit } } = await this.getAccountConfigs({ config: ['CodeSizeLimit'] });
    ...
    const ossObjectName = `${this.accountid}/${objectName}`;
    await client.put(ossObjectName, path.normalize(zipFilePath));
    options.code = { ossBucketName: ossBucket, ossObjectName };

    return await this.publishLayerVersion(layerName, options, headers);
  }
