...
class Client {
  ...
  async request(method, path, query, body, headers = {}, opts = {}) {
    ...
    var isMiniEnv = (typeof(my) != 'undefined' && typeof(my.request) === 'function');
    if (isMiniEnv) {
      headers['content-type'] = 'application/json';
    ...
    var signature = Client.getSignature(
      this.accessKeyID, this.accessKeySecret, method, `/${this.version}${path}`, headers, queriesToSign);
    ...
    var proms;
    if (isMiniEnv) {
      proms = new Promise((resolve, reject) => {
        my.request({
          url: url,
          method: method,
          headers: headers,
