...
describe('client test', function () {
  it('static function getSignature', function () {
    ...
    var signature = FunctionComputeClient.getSignature(ACCESS_KEY_ID, ACCESS_KEY_SECRET, 'GET', '/hello/world', {
      date: 'today'
    }, queries);
    expect(signature).to.be.ok();
    expect(signature).to.contain(`FC ${ACCESS_KEY_ID}:`);
  });
  ...
  it('constructor', function () {
    ...
    expect(client.endpoint).to.be(`http://accountid.${REGION}.fc.aliyuncs.com`);
    ...
    expect(client.endpoint).to.be(`https://accountid.${REGION}.fc.aliyuncs.com`);
    ...
    expect(client.endpoint).to.be(`https://accountid.${REGION}-internal.fc.aliyuncs.com`);
  ...
  describe('service should ok', function () {
    ...
    it('invokeFunction with injected span context should pass traceID to context', async function () {
      ...
      var resp = await client.invokeFunction(serviceName, functionName, 'event', hs);
